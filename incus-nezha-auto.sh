...
# 动态拉取并运行你的最新脚本，同时将输出追加到日志
curl -sLf https://raw.githubusercontent.com/2113087020/incus/main/incus-nezha.sh | bash >> "$LOG_FILE" 2>&1
...
echo "⏰ 正在配置每小时定时任务 (Crontab)..."
# 自动过滤掉旧的同名任务，并添加新任务
(crontab -l 2>/dev/null | grep -v "incus_cron.sh" ; echo "0 * * * * $CRON_SCRIPT") | crontab -

echo "------------------------------------------------"
echo "✅ 恭喜：定时任务已成功激活！"
echo "ℹ️  提示：系统将在每小时的第 0 分钟自动拉取并运行检测重装脚本。"
