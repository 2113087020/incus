...
do_scan() {
    ...
    echo -e "\n${CYAN}=======================================${NC}"
    echo -e "${CYAN}  LXD 容器哪吒探针检测工具${NC}"
    echo -e "${CYAN}=======================================${NC}\n"

    RUNNING_CONTAINERS=$(lxc list -f csv -c ns | grep -i ',RUNNING$' | cut -d',' -f1)
...
select_image() {
    ...
    local image_raw
    image_raw=$(lxc image list -f csv -c lfd 2>/dev/null)
    ...
    local idx=0
    while IFS=',' read -r aliases fingerprint description; do
...
do_rebuild() {
    ...
    for c in "${PICKED_CONTAINERS[@]}"; do
        ...
        # 保存容器的 profiles 和网络配置
        PROFILES=$(lxc config show "$c" 2>/dev/null | grep -A100 '^profiles:' | grep '^ *- ' | sed 's/^ *- //' | tr '\n' ',' | sed 's/,$//')
        if [ -z "$PROFILES" ]; then
            PROFILES="default"
        fi

        # 停止并删除旧容器，用相同名字和配置重建
        if lxc stop "$c" --force 2>/dev/null && \
           lxc delete "$c" 2>/dev/null && \
           lxc init "$SELECTED_IMAGE" "$c" -p ${PROFILES//,/ -p } 2>/dev/null && \
           lxc start "$c" 2>/dev/null; then
    ...
    case $choice in
        1)
            ...
            if [[ "$confirm" =~ ^[Yy]$ ]]; then
                for c in "${INFECTED_CONTAINERS[@]}"; do
                    echo -ne "  正在暂停 ${c}... "
                    if lxc stop "$c" --force 2>/dev/null; then
                        echo -e "${GREEN}已暂停${NC}"
                    else
                        echo -e "${RED}失败${NC}"
                    fi
